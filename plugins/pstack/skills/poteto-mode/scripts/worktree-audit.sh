...
# PR/MR state by branch, fetched once via whichever host CLI applies (see
# docs/gitlab-support.md). Empty if neither gh nor glab is available.
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
git_host_cli=$(bash "$script_dir/../../../scripts/git-host.sh" 2>/dev/null || true)

prs=$(mktemp)
case "$git_host_cli" in
gh)
	gh pr list --author "@me" --state all --limit 1000 \
		--json number,state,headRefName 2>/dev/null > "$prs" || echo "[]" > "$prs"
	;;
glab)
	# Normalize GitLab's MR shape (iid/state/source_branch) to gh's
	# (number/state/headRefName) so the rest of this script stays unchanged.
	glab mr list --author=@me --all --output json 2>/dev/null \
		| jq '[.[] | {number: .iid, state: (.state | ascii_upcase), headRefName: .source_branch}]' \
...
# Transcripts dir: ~/.claude/projects/<encoded-cwd>, where <encoded-cwd> is the
# worktree's path with every "/" (including the leading one) turned into "-".
