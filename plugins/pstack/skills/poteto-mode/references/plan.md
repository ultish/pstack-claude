...
- Pass `model:` explicitly per the configured roles (defaults `claude-opus-5` for both code and judgment, overridden per role via `~/.claude/pstack-models.md` when the user has run `/setup-pstack`).
...
- poteto-mode's **Babysit** playbook after opening the PR.
