...
Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles an automated PR-review bot's comments (GitHub's Copilot review, a GitLab CI review job, or similar) or an agentic security review's comments. The goal is not to ignore the bot by default. The goal is to stop treating every comment as a required code change.

## Decision rubric

Classify each bot-flagged thread before acting:
