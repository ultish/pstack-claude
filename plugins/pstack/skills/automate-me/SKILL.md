...
Locate the active workspace's transcripts before fanning out: `~/.claude/projects/<encoded-cwd>/`, where `<encoded-cwd>` is the working directory with `/` → `-` (leading dash kept). Use only that path. Don't glob across `~/.claude/projects/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Apply the **unslop** skill and the **plugin-dev:skill-development** skill's writing guidelines to every line. Both apply to any agent-read prose, not just skills.
