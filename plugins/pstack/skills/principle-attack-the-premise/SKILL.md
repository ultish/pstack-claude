...
- **Take a census before the next fix.** Count the imbalance per actor. The census shows which actors hold the imbalance, not how large it is. Write the census as a rerunnable script per the **build-the-lever** principle skill.
- **Read the skew.** If the same few actors hold most of the imbalance on every run, something assigns them that role. Find what assigns the role. That assignment is the next "why" per the **fix-root-causes** principle skill.
- **Remove the asymmetry instead of compensating for it**, per the **laziness-protocol** principle skill. Rotate the role between actors, randomize the assignment, or move the role, so that no actor holds it on every run. A return path, a shared pool, a batched hand-off, or a periodic rebalance leaves the assignment in place and adds work on every run.
...
This principle is distinct from the **redesign-from-first-principles** principle skill, which rebuilds a design around a new requirement. It questions a fact the current design assumes.
