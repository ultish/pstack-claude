...
**GitHub only for now.** Step 1's fetch calls use `gh api` against GitHub's REST API directly (`repos/{owner}/{repo}/pulls/{number}/...`). GitLab's merge-request API has a different endpoint shape and field names (`projects/:id/merge_requests/{iid}`, different diff/comment payloads), so this isn't a drop-in `gh`→`glab` swap the way simpler skills are. Detect the host first with `bash "$CLAUDE_PLUGIN_ROOT/scripts/git-host.sh"` (see `docs/gitlab-support.md` in the repo root); if it returns `glab`, tell the user this skill's fetch step needs a GitLab-specific rewrite before it can run against that repo, rather than silently attempting a lossy translation.
...
4. Start a local server on a fixed port:
   ...
   ```
   Run this backgrounded, then navigate to `http://127.0.0.1:8432/pr-review-{number}.html`.
