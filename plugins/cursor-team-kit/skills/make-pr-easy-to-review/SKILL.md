...
Only rewrite history when the user asks for it or agrees to the plan. Before rewriting, detect the git host CLI and fetch the PR/MR's branch and commit info:
...
if [ "$GIT_HOST_CLI" = "gh" ]; then
  ...
  glab mr view <PR> --output json   # title, source_branch, target_branch, state
  glab api "projects/:id/merge_requests/<PR>/commits"   # commit list, fetched separately
fi
git fetch origin <headRefName> <baseRefName>
ORIGINAL_TREE=$(git rev-parse origin/<headRefName>^{tree})
```

Two things differ from a straight `gh`→`glab` swap here:

- `gh pr view --json` takes a caller-chosen field whitelist. `glab mr view --output json`
  has no field-selection flag — it always dumps the full MR object, under GitLab's own
  field names (`source_branch` for `headRefName`, `target_branch` for `baseRefName`).
- GitHub's JSON includes a `commits` array inline. GitLab's MR object doesn't — the commit
  list this workflow needs to plan safe rebase groupings has to come from a separate
  `merge_requests/<PR>/commits` API call.
